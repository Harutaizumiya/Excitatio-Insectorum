...
教师端课堂操作区提供“远程喊话”入口。底部面板内切换自定义喊话与指定学生，设置 1～5 次播报及 10～180 秒展示，预览最终正文。指定学生无座位时提示将直接展示。提交后显示等待展示、展示中、语音异常、回复或结束状态，并可查看本人近期记录与主动结束。

大屏收到指定学生喊话时先高亮座位约 2 秒，然后打开覆盖页面的弹窗；无座位或自定义喊话直接打开弹窗。弹窗仅展示发送教师、正文、剩余时间及“知道了”“自定义回复”。输入回复时本设备暂停倒计时，最多 60 秒；其他设备先回复或教师结束时立即关闭。大屏顶部之外不增加解释性文案，正文以纯文本和大字号呈现。
