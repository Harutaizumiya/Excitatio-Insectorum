...
async function readXlsx(buffer: Buffer): Promise<Array<{ sheetName: string; rows: SourceRow[] }>> {
  ...
  try {
    ...
    throw new BusinessError('STUDENT_IMPORT_FILE_INVALID', 'Excel 文件无法解析', 400);
  }
  ...
  if (sources.length === 0) {
    throw new BusinessError('STUDENT_IMPORT_EMPTY_FILE', '文件中没有可读取的内容', 400);
  }
  return sources;
}
...
function readCsv(buffer: Buffer): { sheetName: string; rows: SourceRow[] } {
  ...
  try {
    ...
    throw new BusinessError('STUDENT_IMPORT_FILE_INVALID', 'CSV 文件无法解析', 400);
  }
...
function parseMappingInput(input?: string): ImportMappingInput | undefined {
  ...
  try {
    ...
    return parsed as ImportMappingInput;
  } catch {
    throw new BusinessError('STUDENT_IMPORT_MAPPING_INVALID', '字段映射必须是有效 JSON', 400);
  }
}
...
export class StudentImportService {
  async parse(
    ...
    if (!file) {
      throw new BusinessError('STUDENT_IMPORT_FILE_REQUIRED', '请上传 Excel 或 CSV 文件', 400);
    }
    if (!file.buffer || file.buffer.length === 0) {
      throw new BusinessError('STUDENT_IMPORT_EMPTY_FILE', '上传文件为空', 400);
    }
    if (file.buffer.length > MAX_IMPORT_FILE_SIZE_BYTES) {
      throw new BusinessError('STUDENT_IMPORT_FILE_TOO_LARGE', '文件大小不能超过 5MB', 400);
    }
  ...
  async importStudents(
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (candidates.length > 0) {
        ...
        const createdResult = await tx.student.createMany({ data });
        created = createdResult.count;
        const persisted = await tx.student.findMany({
          where: {
            classId,
        ...
        createdStudentIds = persisted.map((s) => s.id);
      }
