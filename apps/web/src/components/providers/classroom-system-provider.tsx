...
import { BackendUnavailableAlert } from "./backend-unavailable-alert"
...
function RealtimeQuerySync({ client }: { client: ClassRealtimeClient }) {
  ...
  useEffect(() => {
    ...
    const unsubscribers = [
      ...
      client.subscribe("ANNOUNCEMENT_CHANGED", undefined, (event) => {
        void queryClient.invalidateQueries({ queryKey: classroomQueryKeys.announcements(event.classId) })
      }),
    ]
    return () => {
